...
export default function HistoriqueCard(props: {
  activity: SUPABASE_HISTORIQUE;
}) {
  const { created_at, id, categorie } = props.activity;
  const createdAt = new Date(created_at).toLocaleDateString();

  return (
    <Link
      href={`/historique/${categorie}/${id}`}
      key={id}
      prefetch={false}
      className="flex w-full max-w-[500px] animate-card flex-col gap-y-5 rounded-lg bg-neutral-800 p-4 sm:p-6 md:max-w-[352px] duration-150 lg:hover:shadow-[0_0_8px_rgba(217,184,97,0.25)]"
    >
